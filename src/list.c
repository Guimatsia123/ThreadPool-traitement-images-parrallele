...
/*
 * Returns the largest number of nodes that the list contained until now.
 */
size_t list_size_max(struct list* self) {
  if (self == NULL) {
    list_debug("list_size_max(): self was NULL");
    return 0;
  }
...
struct list* list_concat(struct list* a, struct list* b) {
  ...
  a->size += b->size;
  a->max_size = MAX(a->size, a->max_size);
